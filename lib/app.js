...
wglt.Application.prototype.handleResizeEvent = function () {
    ...
    this.scaleFactor = 1.0;
    if (width > height) {
        this.scaleFactor = Math.max(1, Math.min(Math.floor(width / minMajorAxis), Math.floor(height / minMinorAxis)));
    } else {
        this.scaleFactor = Math.max(1, Math.min(Math.floor(width / minMinorAxis), Math.floor(height / minMajorAxis)));
    }
...
wglt.Application.prototype.isMobile = function() {
