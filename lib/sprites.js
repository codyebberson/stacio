 ...
 */
wglt.SpriteSet.prototype.drawCenteredString = function(str, x, y, opt_color) {
    const x2 = x - ((CHAR_WIDTH * str.length) / 2) | 0;
    this.drawString(str, x2, y, opt_color);
};
...
wglt.SpriteSet.prototype.drawString = function(str, x, y, opt_color) {
    ...
    for (let i = 0; i < lines.length; i++) {
        for (let j = 0; j < lines[i].length; j++) {
            this.drawChar(lines[i].charCodeAt(j), x + j * CHAR_WIDTH, y + i * CHAR_HEIGHT, opt_color);
        }
    }
};
 ...
 */
wglt.SpriteSet.prototype.drawSprites = function(width, height) {
    ...
    this.viewportSizeBuffer[0] = width;
    this.viewportSizeBuffer[1] = height;
